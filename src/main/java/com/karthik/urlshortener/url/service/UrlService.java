...
public class UrlService {
    ...
    public ShortUrlResponse createShortUrl(CreateShortUrlRequest request) {
        ...
        if (request.getCustomAlias() != null && !request.getCustomAlias().isBlank()) {
            customAlias = request.getCustomAlias().trim().toLowerCase();
            if (RESERVED_ALIASES.contains(customAlias)) {
                throw new ReservedAliasException("Reserved alias cannot be used");
            }
        ...
        if (request.getExpiresAt() != null && request.getExpiresAt().isBefore(Instant.now())) {
            throw new InvalidExpirationException("Expiration date cannot be in the past");
        }

        if (customAlias == null) {
            final Url existingUrl = urlRepo.findByOriginalUrlAndUser(request.getOriginalUrl(), user).orElse(null);
            if (existingUrl != null && (existingUrl.getExpiresAt() == null || existingUrl.getExpiresAt().isAfter(Instant.now()))) {
                log.info("Duplicate URL detected for user: {}", user.getEmail());
                return buildShortUrlResponse(existingUrl);
            }
        }
        ...
        final Url savedUrl = urlRepo.save(url);
        ...
        cacheUrl(savedUrl);
        log.info("Short URL created with code: {}", shortCode);

        return buildShortUrlResponse(savedUrl);
    }

    public Url getOriginalUrl(String shortCode) {

        final CachedUrl cachedUrl = redisTemplate.opsForValue().get(shortCode);
        if (cachedUrl != null) {
            validateExpiration(cachedUrl.getExpiresAt());
            urlRepo.incrementClickCount(cachedUrl.getUrlId());
            log.info("Cache hit for shortCode: {}", shortCode);
            return Url.builder()
                    .id(cachedUrl.getUrlId())
                    .shortCode(shortCode)
                    .originalUrl(cachedUrl.getOriginalUrl())
                    .expiresAt(cachedUrl.getExpiresAt())
                    .build();
        }

        log.info("Cache miss for shortCode: {}", shortCode);
        final Url url = urlRepo.findByShortCode(shortCode)
                        .orElseThrow(() ->
    ...
    public Url getOwnedUrlByShortCode(String shortCode) {
    ...
    public PaginatedResponse<UserUrlResponse> getMyUrls(int page, int size) {
        CustomUserDetails currentUser = userService.getAuthenticatedUser();

        User user = userRepo.findById(currentUser.getId()).orElseThrow(() ->
                        new UserNotFoundException("User not found"));

        Pageable pageable = PageRequest.of(page, size);
        Page<Url> urlPage = urlRepo.findByUserOrderByCreatedAtDesc(user, pageable);
        List<UserUrlResponse> responses = urlPage.getContent()
                        .stream()
                        .map(url -> UserUrlResponse.builder()
                                        .shortCode(url.getShortCode())
                                        .shortUrl("https://knurls.me/r/" + url.getShortCode())
        ...
        return PaginatedResponse.<UserUrlResponse>builder()
                .content(responses)
    ...
    private void cacheUrl(Url url) {
        ...
        redisTemplate.opsForValue().set(url.getShortCode(), cachedUrl, CACHE_TTL);
    }

    private void validateExpiration(Url url) {
        validateExpiration(url.getExpiresAt());
    }
    ...
    private ShortUrlResponse buildShortUrlResponse(Url url) {
        return ShortUrlResponse.builder()
                .originalUrl(url.getOriginalUrl())
                .shortCode(url.getShortCode())
                .shortUrl(appProperties.getBaseShortUrl() + url.getShortCode())
                .build();
    }

    private UserUrlResponse buildUserUrlResponse(Url url) {
